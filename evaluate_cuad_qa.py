...
def get_questions_from_csv():
    """
    reads data in category_descriptions.csv and returns them as python dict
    category_descriptions is a csv file , where each line is : Category,Description,Answer Format,Group
    for example : Category: Document Name,Description: The name of the contract,Answer Format: Contract Name,Group: -
    returns dict where dict[Category] = description
    """
...
def get_jaccard(gt, pred):
    """
    this functions computes the jaccard index between gt(ground truth) answer and the pred (predicted answer given by the model)
    """
    remove_tokens = [".", ",", ";", ":"]
...
def load_json(path):
    """
    open a json file at  path , return it as python dict( key, value data structure)
    """
...
def get_answers(test_json_dict):
    """
    takes data from the test_set json file turn them as dict where dict['question_id'] = [list of answers]
    """
    results = {}
...
def get_preds(nbest_preds_dict, conf=None):
    """
    takes nbest_predictions_.json file as a dict and a conf(threshold) 
    return a dict where dict[questionid] = [list of predictions having probability > conf ] 
    """
    results = {}
...
def compute_precision_recall(gt_dict, preds_dict, category=None):
    """
    given gt_dict (the dict where gt_dict["key"] = [list of real_answers]) , 
    and preds_dict(the dict where preds_dict["key"] = [list of predicted answers]) , 
    and category , the category of the question (if we want to compute for only one category)
    compute the precision metric and the recall metric
    """
    tp, fp, fn = 0, 0, 0

    for key in gt_dict:  # for each question in gt_dict
        ...
        if len(answers) == 0:
            if len(preds) > 0:
                # false positive for each one (the model predicts answers(pos) but it's false there's no ans)
                fp += len(preds)
        else:
            for ans in answers:
                assert len(ans) > 0
                ...
                if match_found:
                    # the model gives answer(pos) and it's true there is answer
                    tp += 1
                else:
                    # the model doesn't gives no answer(neg) but it's false there is answser
                    fn += 1
...
def get_precisions_recalls(pred_dict, gt_dict, category=None):
    """
    given the gt_dict (the dict where gt_dict["key"] = [real_answer]) and 
    preds_dict(the dict where preds_dict["key"] = [predicted answers])
    compute list of (precisions , recalls , confs) => conf in [0.99 to 0]
    """
    precisions = [1]
    recalls = [0]
    confs = []
...
def get_aupr(precisions, recalls):
    """
    this functions take list of precisions and recalls for differents conf and return AUPR
    """
    processed_precisions = process_precisions(precisions)
...
def get_results(model_path, gt_dict, verbose=False):
    """
    this functions takes the model_path (example.: "./train_models/roberta-base") ,
    the gt_dict which is the test_set_data as dict and verbose(if True will we print the results)
    and return as a dict , all the metric aupr, precisions and recall , prec_at_90_recall , prec_at_80_recall
    """
    predictions_path = os.path.join(
        model_path, "predict_nbest_predictions.json")
    ...
    results = {"name": name, "aupr": aupr, "prec_at_80_recall": prec_at_80_recall,
               "prec_at_90_recall": prec_at_90_recall}
    return results


def get_results_per_cat(model_path, gt_dict, Category, verbose=False):
    """
    this functions takes the model_path (example.: "./train_models/roberta-base") ,
    the gt_dict which is the test_set_data as dict and verbose(if True will we print the results)
    and return as a dict , all the metric aupr, precisions and recall , prec_at_90_recall , prec_at_80_recall
    ...
    precisions, recalls, confs = get_precisions_recalls(
        pred_dict, gt_dict, Category)
    prec_at_90_recall, _ = get_prec_at_recall(
        precisions, recalls, confs, recall_thresh=0.9)
    prec_at_80_recall, _ = get_prec_at_recall(
        precisions, recalls, confs, recall_thresh=0.8)
...
def save_results(save_dir, model_path, results):
    """
    create a file and write all the results on it
    """
    if not os.path.exists(save_dir):
        os.mkdir(save_dir)
    save_path = os.path.join(
        save_dir, "{}.json".format(model_path.split("/")[-1]))
...
if __name__ == "__main__":
    ...
    results = get_results("./train_models/roberta-base", gt_dict, verbose=True)
    save_results("./results", "./train_models/roberta-base", results)

    results_per_cat = {}
    cat_dict = get_questions_from_csv()
    for key in cat_dict.keys():
        key_results = get_results_per_cat(
            "./train_models/roberta-base", gt_dict, key, verbose=False)
        results_per_cat.update({key: key_results})
    save_results(
        "./results", "./train_models/roberta-base-categories", results_per_cat)
