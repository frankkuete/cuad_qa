...
def main():
    ...
    def get_dataset_pos_mask(dataset):
        ...
        for i in range(len(dataset)):
            ...
            is_positive = end_pos != 0 and start_pos != 0
    ...
    def get_balanced_dataset(dataset):
        ...
        neg_mask = [not mask for mask in pos_mask]
        ...
        keep_mask = [pos_mask[i] or neg_keep_mask[i]
                     for i in range(len(pos_mask))]
        keep_indices = [i for i in range(len(keep_mask)) if keep_mask[i]]

        subset_dataset = train_dataset.select(keep_indices)
    ...
    if training_args.do_train:
        ...
        with training_args.main_process_first(desc="train dataset map pre-processing"):
            for i in range(len(sliced_dataset)):
                sliced_dataset[i] = sliced_dataset[i].map(
        ...
        """
        train_dataset = get_balanced_dataset(train_dataset)
